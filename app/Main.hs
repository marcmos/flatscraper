...
offerFeed = do
  ...
  offers <- return . concat $ catMaybes [otodomOffers, gratkaOffers]
  syncedOffers <- getCurrentTime >>= \t -> syncLastVisitTimestamps t offers
  return $ renderOfferFeed syncedOffers

saveNewsfeed :: IO ()
saveNewsfeed = do
  feed <- offerFeed
  case feed of
    Just x -> T.writeFile "newsfeed.xml" x

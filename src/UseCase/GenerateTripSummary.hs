...
generateAndStoreTripSummaries ::
  ...
  UTCTime ->
  IO ()
generateAndStoreTripSummaries dataAccess routeProvider profileNames cityTransport time = do
  ...
  forM_ matchedCoords $ \(offerId, lat, lon) -> do
    forM_ profileNames $ \profileNameText -> do
      -- Iterate over Text profile names directly
      tripSummaryResult <-
      ...
      case tripSummaryResult of
        ...
        Left err -> putStrLn $ "Failed to generate trip summary with profile " ++ T.unpack profileNameText ++ ": " ++ err
...
generateTripSummaryFromRoute
  ...
  startCoords = do
    ...
    return $ case tripResult of
      ...
      Just (hubName, legs) ->
        generateTripSummary hubName (T.unpack profileName) legs

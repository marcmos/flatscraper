...
import UseCase.Offer (OfferDetailsLoader (..), OfferSeeder (..), OfferView (offerURL))
...
instance OfferSeeder ScrapeListLoader where
  seedOffers (ScrapeListLoader config scraper url) = do
